...
if (max(old$Date) < max(passengers$Date)){
  ...
  new <- merge(old, passengers, all.y = TRUE)
  new <- unique(new)
...
passengers <- passengers %>%
  ...
  mutate(pc = (`2022`/`2019`)*100) %>%
    pivot_longer(`2019`:`2022`, names_to = "Year", 
                 names_pattern = "^(\\d+)", values_to = "Passengers") %>%
    mutate(Passengers = as.numeric(gsub(",", "", Passengers))) %>%
...
f + coord_cartesian(xlim = c(as.Date(format(Sys.Date()-1, "%Y-%m-01"))-months(1), as.Date(format(Sys.Date()-1, "%Y-%m-01"))+months(1)-1)) +
  geom_text_repel(aes(label = if_else(Date == max(Date) & Year == 2022, paste(round(pc,1), "pc\nof 2019", sep = ""), NULL), family = "Verdana"),
                  nudge_x = 6,
                   color = "orangered") +
